...
class DDIMSampler(object):
    ...
    def sample(self,
        ...
        print("jjjjjjjjjjjjjjj",mask.shape)
    ...
    def ddim_sampling(self, cond, shape,
        ...
        for i, step in enumerate(iterator):
            ...
            img_orig = self.model.q_sample(x0, ts,mask = mask)  # TODO: deterministic forward pass?
            new_mask = mask
            adaptive_mask = True
            if adaptive_mask:
                print("ok")
    ...
    #                         weight_map[tmpi,:,:]=1
    #             b,c,h,w = mask.shape
    #             mask_map  = mask.repeat(1,4,1,1)
    #             ones_map = torch.ones(b,73,h,w)
    #             ones_map = ones_map.to(mask.device)
    #             mask_weight = torch.cat([ones_map[:,0,:,:].unsqueeze(dim = 1),mask_map,ones_map[:,1:,:,:]],dim = 1)
